...
export const Dashboard: React.FC<DashboardProps> = ({ showToast }) => {
  ...
  // We need a specific delete mutation if deleting from a different column (which is rare but possible in UI logic)
  // For now, simple implementation assumes we delete mainly what we see, or we use a dynamic hook wrapper if needed.
  // Actually, handleConfirmDelete uses dynamic entity, so we need to call delete on *that* entity. 
  // Refactoring delete logic below.
  ...
  // We need a separate component or logic to handle deletion of ANY entity, 
  // because useMutateEntity is tied to activeTab.
  // Solution: We will pass the delete function to ConfirmModal wrapper or handle it here via direct API for now to keep it simple,
  // OR instantiate a hook dynamically. Since hooks rule of hooks prevents dynamic calls, 
  // we will use the `deleteMutation` if entity == activeTab, or a separate mechanism.
  // Actually, best practice is to have a specialized hook for generic delete or just allow the mutation to handle it.
  // Let's create a specialized Generic Delete for the dashboard flexibility.
  const { remove: genericRemove } = useMutateEntity(itemToDelete?.entity || 'Pessoa'); // Default to avoid hook error
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // If the entity to delete matches the active tab's hook, use it. 
      // If not (e.g. deleting a linked card), the hook above might have re-rendered with the wrong entity 
      // if we relied on `activeTab`. But we instantiated `genericRemove` with `itemToDelete.entity` as key.
      // However, `itemToDelete` changes *after* render. 
      // Safe bet: Use the mutation from the hook we just created which tracks `itemToDelete?.entity`.
